...
//	VkSamplerCreateFlags  	flags				= 0,
	...
	VkSampler sampler;
	vk.device.vkCreateSampler( &sampler_ci, vk.allocator, &sampler ).vkEnforce( null, file, line, func );
	return sampler;
}
...
//	VkBufferViewCreateFlags flags = 0,
	...
	vk.device.vkCreateBufferView( &buffer_view_ci, vk.allocator, &buffer_view ).vkEnforce( null, file, line, func );
	return buffer_view;
}
...
auto createDescriptorPool(
	...
	vk.device.vkCreateDescriptorPool( &pool_create_info, vk.allocator, &descriptor_pool ).vkEnforce( null, file, line, func );
...
auto createSetLayout(
	...
	vk.device.vkCreateDescriptorSetLayout(
		...
		vk.allocator, &descriptor_set_layout ).vkEnforce( null, file, line, func );
...
auto ref addImmutableSampler(
	...
	vkEnforce( !meta.descriptor_set_layout_bindings.empty,
...
private auto ref addDescriptorTypeUpdate(
	...
	static if( is( DESCRIPTOR_TYPE == VkDescriptorImageInfo )) {
		switch( write_set.descriptorType ) {
			case VK_DESCRIPTOR_TYPE_SAMPLER	: vkEnforce( descriptor.sampler != VK_NULL_HANDLE && descriptor.imageView == VK_NULL_HANDLE,
				"VK_DESCRIPTOR_TYPE_SAMPLER requires a VkSampler without VkImageView", file, line, func ); break;
			case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER : vkEnforce( descriptor.sampler != VK_NULL_HANDLE && descriptor.imageView != VK_NULL_HANDLE,
				"VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER requires a VkImageView and VkSampler", file, line, func ); break;
			case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE : vkEnforce( descriptor.sampler == VK_NULL_HANDLE && descriptor.imageView != VK_NULL_HANDLE,
				"VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE requires a VkImageView without VkSampler", file, line, func ); break;
			case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT : vkEnforce( descriptor.sampler == VK_NULL_HANDLE && descriptor.imageView != VK_NULL_HANDLE,
				"VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT requires a VkImageView without VkSampler", file, line, func ); break;
			default : vkEnforce( false,
