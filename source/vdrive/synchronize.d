...
import vdrive.state;
import vdrive.util.util : vkEnforce;
...
auto createFence( ref Vulkan vk, VkFenceCreateFlags fence_create_flags = 0 ) {
	...
	VkFence fence;
	vkCreateFence( vk.device, &fence_create_info, vk.allocator, &fence ).vkEnforce;
	return fence;
}
...
auto createSemaphore( ref Vulkan vk ) {
	...
	vkCreateSemaphore( vk.device, &semaphore_create_info, vk.allocator, &semaphore ).vkEnforce;
	return semaphore;
}
...
auto createEvent( ref Vulkan vk ) {
	...
	VkEvent event;
	vkCreateEvent( vk.device, &event_create_info, vk.allocator, &event ).vkEnforce;
	return event;
}