...
auto listSurfaceFormats( VK_OR_GPU )(
    ...
    ) if( isVulkan!VK_OR_GPU || is( VK_OR_GPU == VkPhysicalDevice )) {

    // extract gpu member based on template argument
    static if( isVulkan!VK_OR_GPU ) {
        VkPhysicalDevice gpu = vk_or_gpu.gpu;
        auto result = Block_Array!VkSurfaceFormatKHR( vk_or_gpu.scratch );
    } else {
        VkPhysicalDevice gpu = vk_or_gpu;
        auto result = Dynamik_Array!VkSurfaceFormatKHR;
    }

    // Enumerate surface formats
    listVulkanProperty!( typeof( result ), vkGetPhysicalDeviceSurfaceFormatsKHR, VkPhysicalDevice, VkSurfaceKHR )( result, file, line, func, gpu, surface );
...
VkSurfaceFormatKHR selectSurfaceFormat( ref Vulkan vk, VkSurfaceKHR surface, VkFormat[] include_formats, bool first_available_as_fallback = true ) {
    auto surface_formats = vk.listSurfaceFormats( surface, false );
    return filterSurfaceFormats( surface_formats, include_formats, first_available_as_fallback );
}
...
auto ref listPresentModes( VK_OR_GPU )(
    ref VK_OR_GPU   vk_or_gpu,
    VkSurfaceKHR    surface,
    bool            print_info = true,
    string          file = __FILE__,
    ...
    // extract gpu member based on template argument
    static if( isVulkan!VK_OR_GPU ) VkPhysicalDevice gpu = vk_or_gpu.gpu;
    else                            VkPhysicalDevice gpu = vk_or_gpu;

    // As the list of available presentation modes is quite short (6 entries) we return the available list in a Static_Array.
    // Static_Array count should not be set to VK_PRESENT_MODE_RANGE_SIZE_KHR, as that count leaves out extension entries.
    ...
    Static_Array!( VkPresentModeKHR, __traits( allMembers, VkPresentModeKHR ).length - 4 ) result;
    listVulkanProperty!( typeof( result ), vkGetPhysicalDeviceSurfacePresentModesKHR, VkPhysicalDevice, VkSurfaceKHR )( result, file, line, func, gpu, surface );
    ...
    if( print_info ) {
        if( result.length == 0 )  {
            ...
            static if( isVulkan!VK_OR_GPU ) auto present_mode_z = Block_Array!char( vk_or_gpu.scratch );
            else                            auto present_mode_z = Dynamic_Array!char();         // allocates
            printf( "VkPresentModeKHR\n=================\n" );
            foreach( present_mode; result ) {
...
auto ref selectPresentMode( ref Vulkan vk, VkSurfaceKHR surface, VkPresentModeKHR[] include_modes, bool first_available_as_fallback = true ) {
    auto present_modes = vk.listPresentModes( surface, false );
    return filter( present_modes, include_modes, first_available_as_fallback );
}
